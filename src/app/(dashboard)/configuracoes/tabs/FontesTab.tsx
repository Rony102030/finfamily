...
export function FontesTab() {
    ...
    const fetchItems = async () => {
        setLoading(true);
        const { data } = await supabase.from('fontes_renda').select('*').eq('user_id', user!.id).order('created_at', { ascending: true });
    ...
    const handleAdd = async () => {
        ...
        const { data, error } = await supabase.from('fontes_renda').insert({
            user_id: user!.id,
            nome: newNome,
            ativo: true
        }).select().single();
    ...
    const handleToggleActive = async (id: string, current: boolean) => {
        const { error } = await supabase.from('fontes_renda').update({ ativo: !current }).eq('id', id);
        if (!error) {
    ...
    const saveEdit = async (id: string) => {
        const { error } = await supabase.from('fontes_renda').update({ nome: editNome }).eq('id', id);
        if (!error) {
            setItems(items.map(i => i.id === id ? { ...i, nome: editNome } : i));
            setEditingId(null);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-lg font-heading font-bold text-white mb-1">Fontes de Renda</h2>
                </div>
                {!isAdding && (
                    <button
            ...
            <div className="space-y-3">
                ...
                {items.map(item => (
                    <div key={item.id} className={`flex flex-col md:flex-row gap-4 py-3 border-b items-center justify-between transition-colors ${item.ativo ? 'border-borders' : 'border-borders/50 opacity-60'}`}>
                        {editingId === item.id ? (
                ...
                {items.length === 0 && !isAdding && !loading && (
                    <div className="text-center py-12 border border-dashed border-borders rounded-2xl">
                        <p className="text-foreground/50">Nenhuma fonte de renda configurada.</p>
                    </div>
                )}
            </div>
        </div>
